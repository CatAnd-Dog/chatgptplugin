...
import config
from model import generate_online, generate_dalle, generate_baidu, generate_plugin, generate_minimax
...
miniKey = oneperfect["mini_access_key"]  # 自定义Key
...
def send_message():
    ...
    # MiniMax
    if model_name == "minimax" and oneperfect["minimax"] and key == miniKey:
        response = Response(stream_with_context(generate_minimax(message)), content_type='text/event-stream')
        return response
